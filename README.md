...
`scripts/build.js` reads `content/*.md`, `data/narrative.json` and
`data/figures.json` and writes one `.html` file per page at the repository
root (`index.html`, `evidence.html`, `what-shipped.html`, `ledger.html`, etc.;
see the `TABS` array at the top of the script for the full list and the
tab-label-to-file mapping) plus `llms.txt`. It needs Node (no other
dependency) and does not touch anything under `content/` or `data/`.

## Two surfaces

`index.html` is the **narrative front door**: a single linear scroll aimed at
a reader who has not decided to care yet, ending at a Devcon call to action.
It is generated by `scripts/narrative.js` from `data/narrative.json`, has its
own shell and its own stylesheet (`assets/narrative.css`), and does not load
`assets/styles.css` at all — so the two token blocks can never collide and the
portal's restrained styling is unaffected by anything done to the front door.

`evidence.html` and the eleven pages behind it are the **evidence portal**,
unchanged. The Overview used to live at `index.html`; it moved to
`evidence.html` when the narrative took the root URL.

### Editing the narrative

Everything the front door says lives in `data/narrative.json`. Each entry in
`scenes` has a `kind` that selects a renderer in `scripts/narrative.js`;
an unknown `kind` fails the build rather than rendering nothing.

Prose fields are markdown, rendered by the same `vendor/minimark.js` the
modules use. Structured widget content (matrix rows, sortable items, ticket
tiers) is JSON, because minimark cannot express a table of typed records and
should not be extended to try.

### Citations: `[[fig:N]]`

Every number on the front door must reference a `data/figures.json` row by id:

```
The rail raised [[fig:156|₹1,025 crore]] across three issuers.
```

At build time that becomes a chip carrying the row's source tier, linking to
`ledger.html#fig-156`, with the full one-line citation in its `title`.
**A missing id throws and fails the build.** Tier colour and staleness are
computed by the same `assets/ledger-core.js` functions the ledger table uses,
so the two surfaces cannot disagree about how strong a source is.

The build also prints a non-fatal warning for every weak-tier, stale or
UNVERIFIED figure the narrative cites. Citing one is allowed — a quarter of
this base is T4/T5 and saying so is the point — but it should be a choice,
so it is visible on every build.

**Figure ids are append-only and must never be reused.** They are now
load-bearing: renumbering a row in `figures.json` silently re-points a
citation at a different claim.

### Adding a widget

1. Add the scene to `data/narrative.json` with a new `kind`.
2. Add a `render<Kind>()` to `scripts/narrative.js` and register it in
   `RENDERERS`.
3. **Render the full content statically**, visible by default. JS may
   restructure or animate it; JS may never be what makes it appear.
4. Add an `enhance<Kind>()` to `assets/narrative.js` that returns early if
   its root element is absent, and scope any hiding CSS under a class that
   JS adds (see `.js-reveal` and `.is-pending`).
...
- **The build must never read the clock.** No `Date.now()`, no bare
  ...
  date". The Devcon countdown is computed client-side for exactly this reason.
...
The site used to be a single-page app: one HTML shell, hash-based
